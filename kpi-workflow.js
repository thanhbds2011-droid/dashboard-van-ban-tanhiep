...
function mount() {
  ...
  section.innerHTML = `
    <div class="kpi-header">
      <div>
        <div><span class="kpi-pilot">PRODUCTION · VẬN HÀNH</span></div>
        <h2>${heading}</h2>
        <p>${description}</p>
...
function periodStatusLabel(period) {
  if (period?.active === true || ['ACTIVE','OPEN','IN_PROGRESS','ASSESSMENT','REPORTING'].includes(clean(period?.status).toUpperCase())) return 'Đang hoạt động';
...
async function loadAll() {
  ...
  try {
    ...
    KpiWorkflowState.period = KpiWorkflowState.periods.find(p => p.status !== 'DELETED' && (p.active === true || ['ACTIVE','OPEN','IN_PROGRESS','ASSESSMENT','REPORTING'].includes(clean(p.status).toUpperCase())))
    ...
    const scopedQuery = (collectionName, ownerField='ownerUserId') => {
      const ref = collection(db, collectionName);
      if (globalRole()) return query(ref, where('periodId','==',periodId));
      if (isLeader()) return query(ref, where('periodId','==',periodId), where('departmentId','==',departmentId));
      return query(ref, where('periodId','==',periodId), where(ownerField,'==',uid));
    };
    const taskQuery = globalRole()
      ...
      : isLeader()
        ...
        : query(collection(db,'tasks'), where('periodId','==',periodId), where('ownerUserId','==',uid));
    const registrationQuery = globalRole()
      ...
      : isLeader()
        ...
        : query(collection(db,'taskRegistrations'), where('periodId','==',periodId), where('userId','==',uid));
    const [usersSnap, tasksSnap, registrationsSnap, evalSnap, commonAllSnap, planSnap] = await Promise.all([
      ...
      getDocs(scopedQuery('taskEvaluations','ownerUserId')),
      getDocs(scopedQuery('commonCriteriaAssessments','userId')),
    ...
    console.error(error);
    message(error?.code === 'permission-denied' ? 'Không đủ quyền đọc dữ liệu KPI. Hãy triển khai firestore.rules và firestore.indexes.json đi kèm bộ Production.' : (error.message || 'Không tải được dữ liệu KPI.'));
  }
}
...
async function createPeriodFromForm(){
  ...
  if(KpiWorkflowState.periods.some(p=>p.active===true||['ACTIVE','OPEN','IN_PROGRESS','ASSESSMENT','REPORTING'].includes(clean(p.status).toUpperCase()))){alert('Đang có một kỳ hoạt động. Hãy kết thúc kỳ hiện tại trước khi mở kỳ mới.');return;}
...
async function deletePeriodData(){
  ...
  const code=prompt(`Nhập chính xác: XOA DU LIEU ${KpiWorkflowState.period.id}`);
  if(code!==`XOA DU LIEU ${KpiWorkflowState.period.id}`){alert('Mã xác nhận không đúng.');return;}
  const periodId=KpiWorkflowState.period.id;
  const collections=['taskEvaluations','commonCriteriaAssessments','kpiPlans','kpiProfiles','kpiAdjustments','taskRegistrations'];
  let count=0;
  for(const name of collections){
    const snap=await getDocs(query(collection(db,name),where('periodId','==',periodId)));
    for(const d of snap.docs){
      await updateDoc(d.ref,{active:false,status:'DELETED',deletedAt:serverTimestamp(),deletedByUserId:KpiWorkflowState.user.uid,deletedByName:KpiWorkflowState.profile.fullName||'',updatedAt:serverTimestamp()});
      count++;
    }
  }
  const taskSnap=await getDocs(query(collection(db,'tasks'),where('periodId','==',periodId)));
  for(const d of taskSnap.docs){
    await updateDoc(d.ref,{active:false,status:'HUY',deletedReason:'Dọn dữ liệu kỳ kiểm thử',deletedAt:serverTimestamp(),deletedByUserId:KpiWorkflowState.user.uid,deletedByName:KpiWorkflowState.profile.fullName||'',updatedAt:serverTimestamp(),updatedByUserId:KpiWorkflowState.user.uid,updatedByName:KpiWorkflowState.profile.fullName||''});
    count++;
  }
  await setDoc(doc(db,'kpiDeletionLogs',`${periodId}_${Date.now()}`),{periodId,deletedCount:count,deleteMode:'SOFT_DELETE',deletedByUserId:KpiWorkflowState.user.uid,deletedByName:KpiWorkflowState.profile.fullName||'',reason:'Dọn dữ liệu kỳ; không xóa vật lý',deletedAt:serverTimestamp()});
  await updateDoc(doc(db,'evaluationPeriods',periodId),{active:false,status:'DELETED',deletedAt:serverTimestamp(),deletedByUserId:KpiWorkflowState.user.uid,updatedAt:serverTimestamp()});
  alert(`Đã vô hiệu hóa ${count} bản ghi phát sinh của ${periodId}. Không xóa vật lý.`);
  await loadAll();
}
function openReport() {
