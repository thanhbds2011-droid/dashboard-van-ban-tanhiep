...
import {
  KPI2B, COMMON_CRITERIA, calculateTaskScore, calculateKpiSummary,
  proposedRating, ratingName, round2
} from './kpi-engine.js?v=20260724.Prod2B';
...
function mount() {
  ...
  section.innerHTML = `
    <div class="kpi-header">
      <div>
        <div><span class="kpi-pilot">PRODUCTION 2B · THÍ ĐIỂM</span></div>
    ...
    <section id="kpiAdminBox" class="kpi-card kpi-admin-danger kpi-hidden kpi-no-print">
      ...
      <div class="kpi-actions">
        <button id="kpiInitPilot" class="kpi-button secondary" type="button">Tạo/mở Quý III/2026</button>
...
async function initializePilotPeriod(){if(!activeRole('ADMIN'))return;await setDoc(doc(db,'evaluationPeriods',KPI2B.PILOT_PERIOD_ID),{name:KPI2B.PILOT_PERIOD_NAME,year:2026,quarter:3,startDate:KPI2B.PILOT_START,endDate:KPI2B.PILOT_END,recommendedPlanningDays:10,autoLockPlan:false,pilotMode:true,status:'ACTIVE',active:true,createdByUserId:state.user.uid,createdAt:serverTimestamp(),updatedAt:serverTimestamp()},{merge:true});await audit('INITIALIZE_PERIOD',{periodId:KPI2B.PILOT_PERIOD_ID});await loadAll();}
