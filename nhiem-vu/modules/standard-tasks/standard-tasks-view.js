...
export async function renderStandardTasksView(outlet) {
  ...
  try {
    ...
    outlet.innerHTML = `<section class="page-card">
      <div class="page-header"><div><h2>${staffMode ? "Đăng ký kế hoạch công việc" : "Danh mục công việc"}</h2><p>${staffMode ? "Chọn các đầu việc dự kiến thực hiện trong kỳ và gửi cấp có thẩm quyền duyệt." : "Tra cứu danh mục công việc theo vị trí việc làm."}</p></div><button id="btnStandardRefresh" class="secondary-button" type="button">↻ Cập nhật</button></div>
