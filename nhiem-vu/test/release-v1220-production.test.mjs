...
test('V1.22.0 version/build/cache are synchronized', () => {
  const ver = read('core/app-version.js');
  assert.match(ver, /APP_VERSION\s*=\s*["']1\.22\.0["']/);
  assert.match(ver, /BUILD_VERSION\s*=\s*["']20260902\.V1_22_0["']/);
  assert.match(ver, /CACHE_NAME\s*=\s*["']nhiem-vu-20260902-v1-22-0["']/);
  assert.match(read('index.html'), /20260902\.V1_22_0/);
  assert.match(read('sw.js'), /20260902\.V1_22_0/);
});
...
test('Accounts and Notification Apps Script snapshots stay on deployed versions', () => {
  assert.match(readRelease('deployment/AppsScript_Accounts_V3.4.3.gs'), /3\.4\.3/);
  assert.match(readRelease('deployment/AppsScript_Notification_AI_Evidence_Archive_V6.4.1.gs'), /6\.4\.1/);
});
