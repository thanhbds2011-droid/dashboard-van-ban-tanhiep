...
test('V1.22.1 version/build/cache are synchronized', () => {
  const ver = read('core/app-version.js');
  assert.match(ver, /APP_VERSION\s*=\s*["']1\.22\.1["']/);
  assert.match(ver, /BUILD_VERSION\s*=\s*["']20260903\.V1_22_1["']/);
  assert.match(ver, /CACHE_NAME\s*=\s*["']nhiem-vu-20260903-v1-22-1["']/);
  assert.match(read('index.html'), /20260903\.V1_22_1/);
  assert.match(read('sw.js'), /20260903\.V1_22_1/);
});
...
test('V1.22.1 checks direct registration authority before optional delegation and optional read fails closed', () => {
  const service = read('services/task-registration-service.js');
...
test('V1.22.1 first catalog delegation write tolerates only permission-denied pre-read and still verifies read-back', () => {
  const service = read('services/standard-task-write-service.js');
...
test('V1.22.1 CDTN milestone path accepts explicit approval delegation without widening general task authority', () => {
  const rules = readRelease('firestore.rules');
...
test('V1.22.1 UI refinement keeps task workspaces full-width and removes generic confirm eyebrow', () => {
  const css = read('ui-v1.22.1.css');
